...
def create_long_snippet(sentences, min_len, max_len):
    ...
    buffer = ""
    for s in sentences:
        if not buffer: buffer = s
        else:
            new_buffer = buffer + ". " + s
            if len(new_buffer) > max_len: continue
            buffer = new_buffer
...
def build_entry(anchor_repo, repo_pool, used_texts):
    # --- إصلاح 1: التأكد من وجود المفاتيح المطلوبة ---
    if "full_name" not in anchor_repo or "description" not in anchor_repo or "readme" not in anchor_repo:
        return None
    ...
    def find_relevance_candidates(logic_func):
        ...
        for repo in repo_pool:
             # --- إصلاح 2: التحقق من المفتاح قبل استخدامه ---
    ...
    # 4. اختيار 4 جمل فريدة لكل مستوى مع خطط بديلة قوية
...
def main():
    ...
    # --- إصلاح 3: فلترة أكثر تساهلاً ---
    usable_repos = []
    for r in repos:
        # التأكد من وجود المفاتيح الأساسية
        if "full_name" not in r or "description" not in r or "readme" not in r:
            r["full_name"] = r.get("full_name", "unknown/repo")
            r["description"] = r.get("description", "")
            r["readme"] = r.get("readme", "")
        
        if len(r.get("description", "") or "") + len(r.get("readme", "") or "") > 100: # تقليل الحد الأدنى
    ...
    if len(usable_repos) < 50: # إذا كان العدد قليل جدًا، نفشل
        print(f"❌ Error: Only {len(usable_repos)} usable repos found. Not enough to build a reliable dataset.", file=sys.stderr)
        sys.exit(1)
    ...
    if len(dataset) < MINIMUM_RECORDS:
         print(f"⚠️ Warning: Could only build {len(dataset)} valid records, which is less than the minimum required {MINIMUM_RECORDS}.", file=sys.stderr)
